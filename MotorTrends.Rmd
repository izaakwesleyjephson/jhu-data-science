...
```{r}
linear_model <- glm(am ~ mpg, data = mtcars)
...
Fitting a linear model gives a slope of `r linear_model$coefficients[2] %>% round(digits = 4)` and intercept of `r linear_model$coefficients[1] %>% round(digits = 3)`. This can be intepreted as an increase of 1 mpg corresponds to a `r linear_model$coefficients[2]  %>% percent(accuracy = 0.1)` increase in the probability that the car has manual transmission.

However, from the graph, we can already see that the linear model is a poor fit for the data. We can see this further by looking at a plot of the residuals, which we would expect to be normally distributed with mean zero, if the model were a good fit. Furthermore, the negative intercept is not readily interpretable.
...
```
As can be seen in the residuals plot, the points are far from normally distributed and we can conclude that this model is a poor fit to the data.

## Logistic model

As we have established that a linear model provides poor fit, we can attempt to fit a logistic regression model to the data. This model is suitable as our dependent variable is binary (either manual or automatic). 
